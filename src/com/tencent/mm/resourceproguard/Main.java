...
public class Main {
	...
	private static TextField mApkPathText;
	...
	private void runGUI() {
		Frame mainFrame = new Frame("ResourceProguard_GUI");
		...
		MenuItem defaultSetting = new MenuItem("默认设置");
		...
		settingMenu.add(defaultSetting);
		...
		Label label1 = new Label("apk路径:",Label.LEFT);
		panel.add(label1);
		mFileDialog = new FileDialog(mainFrame, "打开", FileDialog.LOAD);
		mApkPathText = new TextField(40);
		Button apkSelectBtn = new Button("...");
		
		apkSelectBtn.addMouseListener(new MouseAdapter() {

			@Override
			public void mouseClicked(MouseEvent e) {
				mFileDialog.setDirectory(mRunningLocation);
				mFileDialog.setVisible(true);
				String file = mFileDialog.getFile();
				if(file!=null){
					mApkPathText.setText(mFileDialog.getDirectory()+file);
				}
			}
		});
		panel.add(mApkPathText);
		panel.add(apkSelectBtn);
		Button processBtn = new Button("开始混淆");
		...
		mainFrame.add(panel);
		mDialog = new Dialog(mainFrame, "处理完成",true);
		mDialog.setBounds(300, 400, 200, 50);
		Label dialogLabel = new Label("混淆完成");
		mDialog.setLayout(new FlowLayout(FlowLayout.CENTER));
	...
	private void processProguard() {
		File configFile = null;
		File outputFile = null;
		String apkFileName = mApkPathText.getText();

		if (configFile == null) {
			configFile = new File(mRunningLocation + File.separator
					+ TypedValue.CONFIG_FILE);
		...
		System.out.printf("resourceproguard begin\n");

		mConfiguration = new Configuration(configFile, Main.this);
		...
		ApkDecoder decoder = new ApkDecoder(Main.this);

		File apkFile = new File(apkFileName);
		if (!apkFile.exists()) {
			System.err.printf("the input apk %s does not exit",
		...
		if (outputFile == null) {
			mOutDir = new File(mRunningLocation
					+ File.separator
					+ apkFile.getName().substring(0,
							apkFile.getName().indexOf(".apk")));
		} 
		...
		ResourceApkBuilder builder = new ResourceApkBuilder(Main.this);
		...
		System.out.printf("resources proguard done, total time cost: %fs\n",
				diffTimeFromBegin());
		System.out
				.printf("resources proguard done, you can go to file to find the output %s\n",
	...
	private void run(String[] args) {
		...
		for (int index = 0; index < args.length; index++) {// 读取控制台参数
			String arg = args[index];
			if (arg.equals(ARG_HELP) || arg.equals("-h")) {// 如果参数中包含帮助，就打印操作指引
				goToError();
			} else if (arg.equals(ARG_CONFIG)) {// 参数包含-config，如果该命令后没有.xml文件就打印错误提示及操作指引
				if (index == args.length - 1
						|| !args[index + 1].endsWith(TypedValue.XML_FILE)) {
					System.err
							.println("Missing XML configuration file argument");
					goToError();
				}
				configFile = new File(args[++index]);// 获得配置文件
				if (!configFile.exists()) {// 若不存在这样的路径就打印错误及操作指引
					System.err.println(configFile.getAbsolutePath()
							+ " does not exist");
					goToError();
				}
				System.out.printf("special configFile file path: %s\n",
						configFile.getAbsolutePath());// 若存在配置文件就打印其路径

			} else if (arg.equals(ARG_OUT)) {// 若命令中包含输出路径参数-out
				if (index == args.length - 1) {// 若其后没有参数就打印错误信息及操作指引
			...
			} else if (arg.equals(ARG_SIGNATURE)) {// 检查是否在控制台输入签名信息，包含四个信息：签名文档，storepsw，keypsw，alias
				// 需要检查是否有四个参数
				if (index == args.length - 1) {
					System.err
							.println("Missing signature data argument, should be "
									+ ARG_SIGNATURE
		...
		if (signedFile != null) {
			ResourceRepackage repackage = new ResourceRepackage(this, new File(
					signedFile));
			try {
				if (outputFile != null) {
		...
		try {
			...
			if (mSetSignThroughCmd) {
				mConfiguration.setSignData(signatureFile, keypass, storealias,
						storepass, true);
			}
			...
			if (lineEnd != -1 && (lineEnd - index) < lineWidth) {
				...
				if (lineEnd - index < lineWidth) {
					...
					if (lastSpace > index) {
						...
						// No space anywhere on the line: it contains something
						// wider than
